...
const mockCategories = [
  { id: "cat-1", name: "Politika", slug: "politika", color: "#cf222e", articleCount: 3 },
  { id: "cat-2", name: "Sport", slug: "sport", color: "#1a7f37", articleCount: 5 },
];
...
const validArticle = {
  ...
  content: "Qeveria ka miratuar sot planin e ri ekonomik qe synon rritjen e eksportit dhe investimeve te huaja.",
...
const wrongImageHttp = { ...validArticle, imageUrl: "http://insecure.com/img.jpg" };
...
describe("ArticleForm component — rendering and submission", () => {
  ...
  test("should show error on empty submission", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Ploteso te gjitha fushat e kerkuara.")).toBeInTheDocument();
    });
  ...
  test("should call onSubmit when valid data is filled", async () => {
    ...
    fireEvent.change(selects[0], { target: { value: validArticle.categoryId } });
  ...
  test("should show server error when onSubmit rejects", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Gabim gjate ruajtjes se artikullit.")).toBeInTheDocument();
    });
  });
  ...
  test("should pre-fill form in edit mode and submitted data should match regex", async () => {
    ...
    render(
      <ArticleForm {...defaultProps} initialData={initialData} submitLabel="Ruaj" />
    );
