...
import { createIdGenerator, getArticleWithRelations as joinRelations } from "@gjirafanews/utils";

// Re-export types so existing `@/lib/data` imports still resolve.
...
export function getArticleWithRelations(article: Article): ArticleWithRelations {
  return joinRelations(article, categories, sources);
}
...
// --- Generate unique IDs ---
const generateArticleId = createIdGenerator("art-", articles.length);
export function generateId(): string {
  return generateArticleId();
}

const generateUserIdFn = createIdGenerator("usr-", users.length);
